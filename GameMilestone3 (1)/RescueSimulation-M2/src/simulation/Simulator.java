...
public class Simulator implements WorldListener {
    ...
    private MainLook mainLookListener;//why is it initialized?
    ...
    private void loadUnits(String path) throws Exception {
        BufferedReader br = new BufferedReader(new FileReader("units.csv"));
        ...
        while (line != null) {
            ...
            switch (info[0]) {
                case "AMB": {
                    ...
                    a.setMainLookListener(mainLookListener);
                ...
                case "DCU": {
                    ...
                    d.setMainLookListener(mainLookListener);
                }
                break;
                case "EVC": {
    ...
    private void loadCitizens(String path) throws Exception {
        ...
        while (line != null) {
            ...
            citizens.add(c);
            //mainLookListener.getPanelsinGrid()[x][y].getRescButton().addTarget(c);
            line = br.readLine();
        }
        br.close();
    }
    ...
    private void loadDisasters(String path) throws Exception {
        ...
        while (line != null) {
            ...
            //mainLookListener.addPland(plannedDisasters.get(plannedDisasters.size()-1));
            //plannedDisasters.get(plannedDisasters.size()-1).setMainLookListener(mainLookListener);
            line = br.readLine();
        }
    ...
    public void assignAddress(Simulatable s, int x, int y) {
        ...
        if (s instanceof Citizen) {
            if (((Citizen) s).getLocation() != null) {
                mainLookListener.getPanelsinGrid()[((Citizen) s).getLocation().getX()][((Citizen) s).getLocation().getY()].getRescButton()
                        .getTarget().remove(s);
            }
            ((Citizen) s).setLocation(world[x][y]);
            mainLookListener.getPanelsinGrid()[x][y].getRescButton()
                    .addTarget((Citizen) s);
        } else
    ...
    public void nextCycle() throws DisasterException {
        ...
        currentCycle++;
		int begcausalities=calculateCasualties();

        mainLookListener.getLabels().setNorthLabels(calculateCasualties(), currentCycle);
                ...
                if (d instanceof Fire)
                    ...
                    if (d.getTarget() instanceof Citizen) {
                        ...
                        if(d instanceof Infection)
                            mainLookListener.getInfectionGif().setVisible(true);
                        else
                            mainLookListener.getInjuryGif().setVisible(true);
                        ...
                        d.strike();
                        executedDisasters.add(d);
                        mainLookListener.getCollapsegif().setVisible(true);
        ...
        mainLookListener.getPland().clear();
        mainLookListener.getExd().clear();
        for (int i = 0; i < plannedDisasters.size(); i++) {
            mainLookListener.addPland(plannedDisasters.get(i));
        }
        for (int i = 0; i < executedDisasters.size(); i++) {
            mainLookListener.addexec(executedDisasters.get(i));
        }
        if(calculateCasualties()!=begcausalities)
            mainLookListener.getDeath().setVisible(true);
    ...
    private void handleFire(Disaster d) throws DisasterException {
        ...
        if (b.getGasLevel() == 0) {
            d.strike();
            mainLookListener.getFiregif().setVisible(true);
            executedDisasters.add(d);
    ...
    public boolean checkGameOver() {
       ...
       // mainLookListener.getGameOver().setVisible(true);
    ...
    }public int calculateRescued() {
        int count = 0;
        for (int i = 0; i < citizens.size(); i++) {
            if (citizens.get(i).getState() == CitizenState.RESCUED)
                count++;
        }
        return count;
